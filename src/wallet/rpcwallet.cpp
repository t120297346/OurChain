...
static void SendContractTx(CWallet * const pwallet, const Contract *contract, const CTxDestination &address, CTransactionRef& txNew, const CCoinControl& coin_control, mapValue_t mapValue, std::string fromAccount)
{
    CAmount curBalance = pwallet->GetBalance();
...
UniValue deploycontract(const JSONRPCRequest& request)
{
    ...
    if (!pwallet->GetKeyFromPool(newKey))
        throw JSONRPCError(RPC_WALLET_KEYPOOL_RAN_OUT, "Error: Keypool ran out, please call keypoolrefill first");
    CTxDestination keyID = newKey.GetID();    

    std::string strAccount;
    pwallet->SetAddressBook(keyID, strAccount, "receive");

    // sendtoaddress
    //CTxDestination dest = DecodeDestination(keyID);
    if (!IsValidDestination(keyID)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
    }
    ...
    SendContractTx(pwallet, &contract, keyID, tx, no_coin_control, std::move(mapValue), {} /* fromAccount */);
