...
//LogPrintf("generate success\nnonce =  %u\nnTime= %u\nnTimeNonce= %u\nmaxhash = %s,%s\nnNonce2 = %u\nnTimeNonce2 = %u\nmaxhash2 = %s\nhashMerkleRoot2 = %s\nhashMerkleRoot = %s\nhash = %s\n\n",	pblock->nNonce, pblock->nTime, pblock->nTimeNonce, pblock->maxhash.ToString().c_str(), pblock->GetHash2().ToString().c_str(), pblock->nNonce2, pblock->nTimeNonce2, pblock->maxhash2.ToString().c_str(),pblock->hashMerkleRoot2.ToString().c_str(),pblock->hashMerkleRoot.ToString().c_str(),pblock->GetHash().ToString().c_str());
        if (nMaxTries == 0) {
            break;
        }
